...
print('load json (raw tir info), please wait 20 seconds~')
vid = json.load(open('tir.json', 'r'))
                ...
                for frame_id in seq:
                    ...
                    for obj in frame['objs']:
                        if obj['trackid'] == selected:
                            o = obj
                            continue
                    snippet['{:06d}'.format(int(frame['img_path'].split('.')[0]))] = o['bbox']
                snippets[video['base_path']]['{:02d}'.format(selected)] = snippet
                n_snippets += 1
        print('video: {:d} snippets_num: {:d}'.format(n_videos, n_snippets))
        
train = {k:v for (k,v) in snippets.items() if 'a/' or 'b/' in k}
val = {k:v for (k,v) in snippets.items() if 'c/' in k}
